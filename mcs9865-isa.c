...
static int pci_isa_serial_init_one(struct pci_dev *dev, const struct pci_device_id *ent)
#else
static int __devinit pci_isa_serial_init_one(struct pci_dev *dev, const struct pci_device_id *ent)
#endif
{
	...
	for (i = 0; i < nr_ports; i++) {
		...
		memcpy(&up.port, &serial_port, sizeof(serial_port));
		retval = serial8250_register_8250_port(&up);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,8,0)
static void pci_isa_serial_remove_one(struct pci_dev *dev)
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(3,8,0)
	.remove		= pci_isa_serial_remove_one,
#else
