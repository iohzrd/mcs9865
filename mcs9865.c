...

{
	...
	if (up->custom_setting == 1)
		...
		port->uartclk = DEFAULT9865_BAUD * 16;
...
int serial9865_register_port(struct uart_port *port,struct pci_dev *dev)
{
	...
	if (index >= 0) {
		...
		serial9865_ports[index].custom_setting = 0;
		serial9865_ports[index].custom_baud = 0;
		...
		if(uart_9865_contxts[index].uart_mode == MCS9865_RS485_FULL_DUPLEX || \
			...
			serial9865_ports[index].port.type = PORT_ENHANCED;
